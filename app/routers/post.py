...
def delete_post(id: int, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    if post == None:
        raise HTTPException(detail=f"Could not find post with id {id}", status_code=status.HTTP_404_NOT_FOUND)

    if post.owner_id != current_user.id:
        raise HTTPException(detail=f"You are not authorized to perform this action!", status_code=status.HTTP_403_FORBIDDEN)
